      ...
      (values: any) => {
        if (isEdit) {
          if (txt.replace(/(^\s*)|(\s*$)/g, '') === '') {
            formRef.current?.setFields([{ name: editorName!, errors: ['请输入内容'] }])
            return
          }
        ...
        if (isUpload) {
          if (fileList.length === 0) {
            formRef.current?.setFields([{ name: 'file', errors: ['请上传图片'] }])
            return
          }
          ...
          if (notTrueFile.length > 0) {
            formRef.current?.setFields([{ name: 'file', errors: ['请上传指定格式文件'] }])
            return
          }
          // 判断文件大小
          ...
          if (notTrueSizeFile.length > 0) {
            formRef.current?.setFields([{ name: 'file', errors: ['文件过大'] }])
            return
          }
      ...
      >
        {options.map((item: formType, index: number) => {
          ...
          return item.Custom ? (
            ...
            >
              {/* 开关 */}
              {item.type === 'switch' ? (
                <Switch checkedChildren={item.openTxt} unCheckedChildren={item.closeTxt} />
              ) : null}
            </Form.Item>
          ) : (
            <Form.Item
            ...
            >
              {/* 普通输入框 */}
              {item.type === 'input' ? <Input placeholder={item.placeholder}></Input> : null}
              {/* 时间 */}
              {item.type === 'timeDefault' ? <TimePicker format={item.format}></TimePicker> : null}
              {/* 日期范围 */}
              {item.type === 'timeRange' ? <RangePicker format={item.format} showTime /> : null}
              {/* 多选框 */}
              {item.type === 'select' ? (
                <Select {...attr} style={{ width: 300 }} placeholder={item.placeholder}>
                  {item.data?.map((data: any) => {
                    return (
                      <Select.Option value={data[item.dataValue!]} key={data.id}>
                        {data.dataName}
                      </Select.Option>
                    )
                  })}
        ...
        <Form.Item wrapperCol={{ offset: 8, span: 16 }}>
          <Space wrap>
            ...
            <Button onClick={cancel} type={cancelBtn.type} danger={cancelBtn.isDanger}>
              {cancelBtn.txt}
            </Button>
          </Space>
        </Form.Item>
