import { LocalFileType } from '@renderer/types/coderBox'
...
export async function getCssLibs(this: MinicoderBox, config: LocalFileType): Promise<string[]> {
  ...
  return [
    ...
    .filter((text) => !!text)
    .map((text) => ElementGenerator(text, 'style'))
}

export async function getJsLibs(this: MinicoderBox, config: LocalFileType): Promise<string[]> {
  const { resource } = this
  ...
  return [
    ...
    .filter((text) => !!text)
    .map((text) => ElementGenerator(text, 'script'))
}

export async function getScriptForTab(this: MinicoderBox, config: LocalFileType): Promise<string> {
